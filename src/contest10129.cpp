...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg){
	...
	if (laserIndex - laserSize/2 - laserOffset != laserSize/2 + laserOffset-laserIndex){
		if (laserIndex - laserSize/2 - laserOffset > laserSize/2 + laserOffset-laserIndex)
			leftTurn = !leftTurn;
		else
			rightTurn = !rightTurn;	
...
int main(int argc, char **argv)
{
	...
	*/
	...
	//std::cout<<walldetector;

	while (walldetector == 0) {

		
		for (int i = 0; i <= 100; i++) { // loop if turtlebot is not // to wall
			
			ros::spinOnce(); // invoke all callback funcs and publish msgs
			//std::cout<<i;


			if (laserRange >=0.7 && i==0) { // keep straight when no wall detected
				angular = 0;
				linear = 0.18;
			...
			/*if (laserRange < 0.7 && leftTurn == 1 && i==0){ // if wall on the left side of turtlebot, turn 90 deg to keep wall on the right
				angular = -pi/2;
				linear = 0;
				std::cout<<"entered left turn";
				break;
			} */
				...
				if (i == 0){
					...
					if (d[i] <= d[i-1]){ // if laserfront distance increases, keep turning left
						angular = -pi/20;
						linear = 0;
					}
					if (d[i] > d[i-1]){ // until laserfront reading start to decreases, turtlebot is now perpendicular to wall
						angular = 0; // turn 90 degree to make turtlebot parallet to wall
						linear = 0;
						yawZero = yaw;
						walldetector = 1;
						break;
