...
bool infState=false;
ros::Time infStart;
ros::Duration infDiff;
double infDiffSec;
bool wowPlayed = false;
...
{	
	...
	while(ros::ok()){
		...
		if(cliffCenter && cliffLeft && cliffRight){
		...
		else if(follow_cmd.angular.z == 0 &&  follow_cmd.linear.x == 0){  //lose track of object for 3 sec, case 1
			lost_counter ++;
			if (lost_counter == 1) {
				start = ros::Time::now();
			} else {
		...
		if(world_state == 0){	 // object detected, continue following
			...
			if(infState==0){
				infStart = ros::Time::now();
				infState=1;
			}
			else{
				infDiff = ros::Time::now() - infStart;
				infDiffSec = infDiff.toSec();
				if(wowPlayed==false){
					destroyAllWindows();
				    waitKey(50);
				...
				if(whistlePlayed==false && infDiffSec > 3){
		...
		if(infState==0) world_state=0;  //----------------------continue follower function after action---------------------------
	}
